...
/**
 * Test which verifies that the sink correctly handles keyframe synchronization.
 */
void test_rdpecam_sink__keyframe_sync(void) {
    guac_client* client = create_mock_client();
    CU_ASSERT_PTR_NOT_NULL_FATAL(client);
    ...
    /* Request keyframe (should already be set by default, but be explicit) */
    guac_rdpecam_request_keyframe(sink);
    ...
    /* Push another non-keyframe */
    create_frame_header(&header, sizeof(payload), 1033, false);
    memcpy(frame_data, &header, sizeof(header));
    memcpy(frame_data + sizeof(header), payload, sizeof(payload));
    CU_ASSERT_TRUE(guac_rdpecam_push(sink, frame_data, sizeof(frame_data)));

    /* Push a keyframe */
    create_frame_header(&header, sizeof(payload), 1066, true);
    memcpy(frame_data, &header, sizeof(header));
    memcpy(frame_data + sizeof(header), payload, sizeof(payload));
    CU_ASSERT_TRUE(guac_rdpecam_push(sink, frame_data, sizeof(frame_data)));

    /* Push another non-keyframe after the keyframe */
    create_frame_header(&header, sizeof(payload), 1100, false);
    memcpy(frame_data, &header, sizeof(header));
    memcpy(frame_data + sizeof(header), payload, sizeof(payload));
    CU_ASSERT_TRUE(guac_rdpecam_push(sink, frame_data, sizeof(frame_data)));

    CU_ASSERT_EQUAL(guac_rdpecam_get_queue_size(sink), 4);

    /* Pop - should skip first two and return the keyframe */
    uint8_t* out_buf = NULL;
    size_t out_len = 0;
    bool keyframe = false;
    uint32_t pts_ms = 0;
    
    CU_ASSERT_TRUE(guac_rdpecam_pop(sink, &out_buf, &out_len, &keyframe, &pts_ms));
    CU_ASSERT_PTR_NOT_NULL(out_buf);
    CU_ASSERT_TRUE(keyframe);
    CU_ASSERT_EQUAL(pts_ms, 1066);
    guac_mem_free(out_buf);

    /* Next pop should return the subsequent non-keyframe (sync achieved) */
    CU_ASSERT_TRUE(guac_rdpecam_pop(sink, &out_buf, &out_len, &keyframe, &pts_ms));
    CU_ASSERT_PTR_NOT_NULL(out_buf);
    CU_ASSERT_FALSE(keyframe);
    CU_ASSERT_EQUAL(pts_ms, 1100);
    guac_mem_free(out_buf);
...
/**
 * Test which verifies that the sink can be cleared.
 */
void test_rdpecam_sink__clear(void) {
    guac_client* client = create_mock_client();
    CU_ASSERT_PTR_NOT_NULL_FATAL(client);
    ...
    /* Push some frames */
    for (int i=0; i<5; i++) {
        create_frame_header(&header, sizeof(payload), 1000 + i*33, false);
        memcpy(frame_data, &header, sizeof(header));
        memcpy(frame_data + sizeof(header), payload, sizeof(payload));
        guac_rdpecam_push(sink, frame_data, sizeof(frame_data));
    }

    CU_ASSERT_EQUAL(guac_rdpecam_get_queue_size(sink), 5);

    /* Clear the sink */
    guac_rdpecam_clear(sink);
    CU_ASSERT_EQUAL(guac_rdpecam_get_queue_size(sink), 0);
