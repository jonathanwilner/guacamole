...
void guac_rdp_rdpecam_caps_notify(guac_client* client) {
    ...
    if (new_device_count == 0) {
        if (plugin->device_id_map && !HashTable_GetItemValue(plugin->device_id_map, GUAC_RDPECAM_VIRTUAL_DEVICE_ID)) {
    ...
    /* 3. If real cameras added, add them (mapping_add handles existing) */
        ...
        for (unsigned int i = 0; i < new_device_count; i++) {
            ...
            if (plugin->device_id_map && HashTable_GetItemValue(plugin->device_id_map, caps->device_id))
                continue;
        ...
        if (!found_slot) {
            guac_client_log(client, GUAC_LOG_ERROR,
                    "RDPECAM no available channel slots for new device '%s'", caps->device_id);
            continue;
        }
...
static void guac_rdp_rdpecam_mapping_remove_by_device_id(
    ...
    while (current) {
        if (current->device_id_key && strcmp(current->device_id_key, device_id) == 0) {
            ...
            guac_mem_free((void*) current->device_id_key);
...
#ifdef HAVE_WINPR_HASHTABLE_INSERT
    if (!HashTable_Insert(plugin->device_id_map, (void*) id_copy, (void*) entry)) {
#else
    if (HashTable_Add(plugin->device_id_map, (void*) id_copy, (void*) entry) < 0) {
#endif
        guac_mem_free(id_copy);
        guac_mem_free(channel_copy);
        guac_mem_free(entry);
...
void guac_rdp_rdpecam_send_device_notifications(
    ...
    if (device_count == 0) {
        guac_client_log(client, GUAC_LOG_DEBUG,
                "RDPECAM no devices reported by browser, advertising default virtual camera");
        ...
        if (plugin->manager) {
            ...
            if (device_listener) {
                ...
                if (saved_channel_name) {
                    ...
                    guac_client_log(client, GUAC_LOG_DEBUG,
                            "RDPECAM registered listener for default device channel: %s", channel_name);
                } else {
                    guac_mem_free(device_listener);
                }
            }
        ...
        if (rs && rdpecam_build_device_added(rs, device_name, channel_name)) {
            ...
            guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPECAM TX ChannelId=%" PRIu32 " MessageId=0x05 DeviceAddedNotification (default virtual camera) result=%u",
    ...
    guac_client_log(client, GUAC_LOG_DEBUG,
            "RDPECAM sending DeviceAddedNotification for %u real device(s)", device_count);
    ...
    for (unsigned int i = 0; i < device_count; i++) {
        ...
        if (rs && rdpecam_build_device_added(rs, device_name, channel_name)) {
            ...
            guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPECAM TX ChannelId=%" PRIu32 " MessageId=0x05 DeviceAddedNotification (device='%s', channel='%s') result=%u",
