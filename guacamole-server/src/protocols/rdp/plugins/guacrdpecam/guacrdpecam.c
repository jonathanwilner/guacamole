...
static void guac_rdp_rdpecam_mapping_clear(guac_rdp_rdpecam_plugin *plugin);
static void
guac_rdp_rdpecam_mapping_remove_by_channel(guac_rdp_rdpecam_plugin *plugin,
                                           const char *channel_name);
static void
guac_rdp_rdpecam_mapping_remove_by_device_id(guac_rdp_rdpecam_plugin *plugin,
                                             const char *device_id);
static BOOL guac_rdp_rdpecam_mapping_add(guac_rdp_rdpecam_plugin *plugin,
                                         const char *device_id,
                                         const char *channel_name);
 ...
 */
static void guac_rdp_rdpecam_log_hexdump(guac_client *client,
                                         const char *direction,
                                         const char *channel_name,
                                         UINT32 channel_id, const BYTE *data,
                                         size_t length) {
  ...
  if (length > max_dump) {
    guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPECAM %s %s[id=%" PRIu32
                    "] hexdump length=%zu truncated to %zu bytes",
                    direction ? direction : "",
                    channel_name ? channel_name : "rdpecam", channel_id, length,
                    dump_len);
  }
  ...
  for (size_t offset = 0; offset < dump_len; offset += 16) {
    ...
    guac_client_log(client, GUAC_LOG_DEBUG,
                    ...
                    direction ? direction : "",
                    channel_name ? channel_name : "rdpecam", channel_id,
                    (uint64_t)offset, hexbuf, ascii);
  }
}

static void guac_rdp_rdpecam_log_message(
    guac_client *client, const char *prefix, const char *channel_name,
    UINT32 channel_id, UINT8 cam_msg, size_t payload_len, const BYTE *payload) {
  ...
  if (guac_rdp_rdpecam_should_hexdump())
    guac_rdp_rdpecam_log_hexdump(client, prefix, channel_name, channel_id,
                                 payload, payload_len);
}

static void guac_rdp_rdpecam_log_stream(guac_client *client, const char *prefix,
                                        const char *channel_name,
                                        UINT32 channel_id, wStream *stream) {
...
guac_rdpecam_device_create(guac_rdp_rdpecam_plugin *plugin,
...
void guac_rdp_rdpecam_caps_notify(guac_client *client) {
  ...
  /* 1. Remove devices that are no longer present in capabilities */
  guac_rdp_rdpecam_device_mapping *mapping = plugin->device_id_mappings;
  while (mapping) {
    /* Need to save next pointer before possible removal */
    guac_rdp_rdpecam_device_mapping *next_mapping = mapping->next;
    bool found = false;
    const char *device_id = mapping->device_id_key;

    /* If this is the virtual camera, it's 'found' if we have no other cameras
     */
    if (device_id && strcmp(device_id, GUAC_RDPECAM_VIRTUAL_DEVICE_ID) == 0) {
      if (new_device_count == 0)
        found = true;
    } else {
      /* Check if this real device is still in the new caps */
      for (unsigned int i = 0; i < new_device_count; i++) {
        guac_rdp_rdpecam_device_caps *caps =
            &rdp_client->rdpecam_device_caps[i];
        if (caps->device_id && device_id &&
            strcmp(caps->device_id, device_id) == 0) {
          found = true;
          break;
        }
      }
    ...
    if (!found) {
      ...
      guac_client_log(
          ...
          "RDPECAM device '%s' (channel '%s') removed from capabilities",
          device_id, channel_name);

      /* Send DeviceRemovedNotification */
      ...
      if (rs && rdpecam_build_device_removed(rs, channel_name)) {
        ...
        if (plugin->manager && plugin->manager->GetChannelId)
          enum_channel_id =
              plugin->manager->GetChannelId(plugin->enumerator_channel);

        pthread_mutex_lock(&(rdp_client->message_lock));
        UINT result = plugin->enumerator_channel->Write(
            plugin->enumerator_channel, (UINT32)out_len, Stream_Buffer(rs),
            NULL);
        pthread_mutex_unlock(&(rdp_client->message_lock));

        guac_client_log(client, GUAC_LOG_DEBUG,
                        "RDPECAM TX ChannelId=%" PRIu32
                        " MessageId=0x06 DeviceRemovedNotification "
                        "(channel='%s') result=%u",
      ...
      /* Clean up device structure if it exists */
      guac_rdpecam_device *device =
          (guac_rdpecam_device *)HashTable_GetItemValue(plugin->devices,
                                                        (void *)channel_name);
      ...
      if (device) {
        ...
        HashTable_Remove(plugin->devices, (void *)channel_name);
        guac_rdpecam_device_destroy(device);
      }

      /* Remove mapping entry (frees channel_name and device_id_key) */
  ...
  /* 3. If real cameras added, add them (mapping_add handles existing) */
    ...
    for (unsigned int i = 0; i < new_device_count; i++) {
      ...
      for (unsigned int check_idx = 0; check_idx < 100; check_idx++) {
        snprintf(channel_name, sizeof(channel_name), "RDCamera_Device_%u",
                 check_idx);

        /* Check if this channel name is in use by any mapping */
        bool in_use = false;
        guac_rdp_rdpecam_device_mapping *m = plugin->device_id_mappings;
        while (m) {
          if (m->channel_name && strcmp(m->channel_name, channel_name) == 0) {
            in_use = true;
            break;
          }
          m = m->next;
      ...
      if (found_slot) {
        ...
        guac_client_log(
            client, GUAC_LOG_DEBUG,
            "RDPECAM adding new device '%s' (ID '%s') to channel '%s'",
            device_name, caps->device_id, channel_name);

        /* Store device ID to channel name mapping */
        if (!guac_rdp_rdpecam_mapping_add(plugin, caps->device_id,
                                          channel_name)) {
        ...
        if (rs && rdpecam_build_device_added(rs, device_name, channel_name)) {
          ...
          guac_client_log(client, GUAC_LOG_DEBUG,
                          "RDPECAM TX ChannelId=%" PRIu32
                          " MessageId=0x05 DeviceAddedNotification "
                          "(device='%s', channel='%s')",
  ...
  guac_client_log(
      client, GUAC_LOG_DEBUG,
      "RDPECAM caps_notify: completed incremental capability update");
}
 ...
 *     Always NULL.
 */
static void *guac_rdp_rdpecam_send_camera_start_signal_callback(guac_user *user,
                                                                void *data) {
  ...
  int concise_written =
      snprintf(concise, sizeof(concise), "%ux%u@%u/%u#%u#%s", params->width,
               params->height, params->fps_numerator, params->fps_denominator,
               params->stream_index, device_id_str);
 ...
 *     Always NULL.
 */
static void *guac_rdp_rdpecam_send_camera_stop_signal_callback(guac_user *user,
                                                               void *data) {
 ...
 *
 * @param arg
 *     Pointer to the guac_rdpecam_device structure for per-device frame
 * processing.
 *
 * @return
 *     NULL on success.
...
static void *guac_rdp_rdpecam_dequeue_thread(void *arg) {
  ...
  guac_client_log(client, GUAC_LOG_DEBUG,
                  "RDPECAM dequeue thread started for device: %s",
                  device->device_name);
  ...
  while (true) {
    ...
    bool keyframe = false;
    uint32_t pts_ms = 0;
    if (!guac_rdpecam_pop(sink, &frame_data, &frame_length, &keyframe,
                          &pts_ms)) {
    ...
    if (!frame_data || frame_length == 0) {
      guac_client_log(client, GUAC_LOG_WARNING,
                      "RDPECAM received invalid frame data");
      if (frame_data) {
    ...
    if (s && rdpecam_write_sample_response_header(
                 s,
                 /*streamId*/ stream_idx,
                 /*sampleSequence*/ sample_seq, (uint32_t)frame_length,
      ...
      pthread_mutex_unlock(&device->lock);
      guac_rdp_rdpecam_log_stream(client, "TX", device->device_name,
                                  log_channel_id, s);

      /* Use message_lock to prevent blocking the RDP event loop */
      guac_rdp_client *rdp_client = (guac_rdp_client *)client->data;
      pthread_mutex_lock(&(rdp_client->message_lock));
      UINT result = active_channel->Write(active_channel, (UINT32)sample_len,
      ...
      if (result == CHANNEL_RC_OK) {
        ...
        guac_client_log(client, GUAC_LOG_DEBUG,
                        "RDPECAM frame sent: %zu bytes, keyframe=%s, pts=%u "
                        "ms, credits %u->%u",
                        frame_length, keyframe ? "yes" : "no", pts_ms, before,
                        remaining);

      } else {
        /* DVC Write failed - log detailed information */
        guac_client_log(client, GUAC_LOG_WARNING,
                        "RDPECAM DVC Write FAILED: size=%zu, result=0x%08X, "
                        "frame=%zu, keyframe=%s",
        ...
        if (result != CHANNEL_RC_OK) {
          guac_client_log(
              ...
              "RDPECAM channel write failed (code=0x%08X), stopping streaming",
              result);
          pthread_mutex_lock(&device->lock);
          device->streaming = false;
      ...
      if (s)
        Stream_Free(s, TRUE);
      guac_client_log(client, GUAC_LOG_ERROR,
                      "RDPECAM failed to build sample header");
    }
    ...
    if (frames_processed % 100 == 0 || (current_time - last_stats_time) >= 30) {
      ...
      float drop_rate = total_frames > 0
                            ? (float)frames_dropped * 100.0f / total_frames
                            : 0.0f;
      ...
      guac_client_log(client, GUAC_LOG_INFO,
                      "RDPECAM performance stats: device=%s, processed=%u, "
                      "dropped=%u, drop_rate=%.1f%%, credits=%u, queue=%d/%d",
                      device->device_name, frames_processed, frames_dropped,
                      drop_rate, device_credits_log,
                      guac_rdpecam_get_queue_size(sink),
  ...
  uint32_t total_frames = frames_processed + frames_dropped;
  float drop_rate =
      total_frames > 0 ? (float)frames_dropped * 100.0f / total_frames : 0.0f;
  guac_client_log(client, GUAC_LOG_INFO,
                  "RDPECAM final stats for device=%s: processed=%u, "
                  "dropped=%u, drop_rate=%.1f%%",
                  device->device_name, frames_processed, frames_dropped,
                  drop_rate);

  guac_client_log(client, GUAC_LOG_DEBUG,
                  "RDPECAM dequeue thread stopped for device: %s",
                  device->device_name);
 ...
 */
static UINT guac_rdp_rdpecam_handle_data(
    guac_client *client, IWTSVirtualChannel *channel, wStream *stream,
  ...
  guac_rdpecam_device *device =
      rdpecam_channel_callback ? rdpecam_channel_callback->device : NULL;
  guac_rdp_rdpecam_plugin *plugin =
      rdpecam_channel_callback ? rdpecam_channel_callback->plugin : NULL;
  const char *ch_name = rdpecam_channel_callback
                            ? rdpecam_channel_callback->channel_name
                            : "unknown";
  UINT32 channel_id =
      rdpecam_channel_callback ? rdpecam_channel_callback->channel_id : 0;

  /* Get remaining data from current stream position (FreeRDP has already
   * consumed any framing) */
  size_t data_length = Stream_GetRemainingLength(stream);

  if (data_length < 2) {
    guac_client_log(
        client, GUAC_LOG_WARNING,
        "RDPECAM message too short: %zu bytes (expected at least 2 for header)",
        data_length);
  ...
  guac_client_log(client, GUAC_LOG_DEBUG,
                  "RDPECAM RX message on %s[id=%" PRIu32
                  "]: version=%d, cam_msg=0x%02x, payload_len=%zu",
                  ch_name, channel_id, version, cam_msg, data_length - 2);

  /* Verify protocol version */
  if (version != RDPECAM_PROTO_VERSION) {
    guac_client_log(client, GUAC_LOG_WARNING,
                    "RDPECAM received message with unexpected version: "
  ...
  /* Stream is now positioned at the start of the payload (after version and
   * messageId) */
  ...
  case RDPECAM_MSG_SELECT_VERSION_RESPONSE: {
    ...
    guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPECAM RX ChannelId=%" PRIu32
                    " MessageId=0x04 SelectVersionResponse (version=%d)",
    ...
    /* Always send device notifications once version is negotiated.
     * If no real devices exist, this will advertise the default virtual camera.
     */
    if (plugin && plugin->enumerator_channel) {
      guac_rwlock_acquire_write_lock(&(rdp_client->lock));
      guac_rdp_rdpecam_send_device_notifications(plugin, client, rdp_client,
                                                 plugin->enumerator_channel);
  ...
  case RDPECAM_MSG_ACTIVATE_DEVICE_REQUEST: {
    guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPECAM received ActivateDeviceRequest on %s[id=%" PRIu32
                    "]",
                    ch_name, channel_id);
    ...
    if (!strcasecmp(ch_name, GUAC_RDPECAM_CHANNEL_NAME)) {
      ...
      if (rs && rdpecam_build_success_response(rs)) {
        ...
        guac_client_log(
            client, GUAC_LOG_DEBUG,
            "RDPECAM TX ChannelId=%" PRIu32
            " MessageId=0x01 SuccessResponse (ActivateDevice control)",
            channel_id);
      }
    } else {
      if (!device) {
        guac_client_log(client, GUAC_LOG_WARNING,
                        "RDPECAM ActivateDevice on device channel but no "
                        "device available (ChannelId=%" PRIu32 ")",
                        channel_id);
        return CHANNEL_RC_OK;
      }
      ...
      if (rs && rdpecam_build_success_response(rs)) {
        ...
        guac_client_log(
            client, GUAC_LOG_DEBUG,
            "RDPECAM TX ChannelId=%" PRIu32
            " MessageId=0x01 SuccessResponse (ActivateDevice device=%s)",
  ...
  case RDPECAM_MSG_DEACTIVATE_DEVICE_REQUEST: {
    guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPECAM received DeactivateDeviceRequest on %s[id=%" PRIu32
                    "]",
                    ch_name, channel_id);
    ...
    if (!strcasecmp(ch_name, GUAC_RDPECAM_CHANNEL_NAME)) {
      ...
      if (rs && rdpecam_build_success_response(rs)) {
        ...
        guac_client_log(
            client, GUAC_LOG_DEBUG,
            "RDPECAM TX ChannelId=%" PRIu32
            " MessageId=0x01 SuccessResponse (DeactivateDevice control)",
            channel_id);
      }
    } else {
      if (!device || !device->sink) {
        guac_client_log(client, GUAC_LOG_WARNING,
                        "RDPECAM DeactivateDevice on device channel but no "
                        "device/sink available (ChannelId=%" PRIu32 ")",
                        channel_id);
        return CHANNEL_RC_OK;
      }

      bool same_stream_channel = (device->stream_channel == channel) ||
                                 rdpecam_channel_callback->is_stream_channel;
      if (!same_stream_channel) {
        ...
        if (rs && rdpecam_build_success_response(rs)) {
          ...
          guac_client_log(client, GUAC_LOG_DEBUG,
                          "RDPECAM TX ChannelId=%" PRIu32
                          " MessageId=0x01 SuccessResponse (DeactivateDevice "
                          "property device=%s)",
                          channel_id, device->device_name);
        }
        break;
      ...
      if (was_active_sender) {
        ...
        guac_client_log(client, GUAC_LOG_DEBUG,
                        "RDPECAM NOT clearing shared sink state (device=%s was "
                        "not active sender)",
                        device->device_name);
      }
      ...
      for (uint32_t i = 0; i < outstanding; i++) {
        ...
        if (es && rdpecam_build_sample_error_response(es, stream_index)) {
          ...
          guac_client_log(client, GUAC_LOG_DEBUG,
                          "RDPECAM TX ChannelId=%" PRIu32
                          " MessageId=0x13 SampleErrorResponse (stream=%u)",
      ...
      if (rs && rdpecam_build_success_response(rs)) {
        ...
        guac_client_log(
            client, GUAC_LOG_DEBUG,
            "RDPECAM TX ChannelId=%" PRIu32
            " MessageId=0x01 SuccessResponse (DeactivateDevice device=%s)",
            channel_id, device->device_name);

        /* Only inform browser to stop camera if this device was the active
         * sender. This prevents stopping the wrong camera when multiple devices
         * exist but only one is actively streaming. */
        if (was_active_sender) {
          guac_client_log(client, GUAC_LOG_DEBUG,
                          "RDPECAM sending camera-stop to browser (device %s "
                          "was active sender)",
                          device->device_name);
          guac_client_for_owner(
              client, guac_rdp_rdpecam_send_camera_stop_signal_callback, NULL);
        } else {
          guac_client_log(client, GUAC_LOG_DEBUG,
                          "RDPECAM NOT sending camera-stop to browser (device "
                          "%s was not active sender)",
                          device->device_name);
        }
      }
    }
  ...
  case RDPECAM_MSG_STREAM_LIST_REQUEST: {
    if (!device) {
      guac_client_log(client, GUAC_LOG_WARNING,
                      "RDPECAM StreamListRequest received but no device "
                      "available on ChannelId=%" PRIu32,
                      channel_id);
      return CHANNEL_RC_OK;
    }
    ...
    guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPECAM received StreamListRequest");

    rdpecam_stream_desc stream = {.FrameSourceType =
    ...
    if (rs && rdpecam_build_stream_list(rs, &stream, 1)) {
      ...
      guac_client_log(client, GUAC_LOG_DEBUG,
                      "RDPECAM TX ChannelId=%" PRIu32
                      " MessageId=0x0A StreamListResponse (streams=%u)",
                      channel_id, 1U);
    }
    break;
  }
  ...
  case RDPECAM_MSG_MEDIA_TYPE_LIST_REQUEST: {
    if (!device) {
      guac_client_log(client, GUAC_LOG_WARNING,
                      "RDPECAM MediaTypeListRequest received but no device "
                      "available on ChannelId=%" PRIu32,
                      channel_id);
      return CHANNEL_RC_OK;
    }
    ...
    if (payload_len < 1) {
      guac_client_log(client, GUAC_LOG_WARNING,
                      "RDPECAM MediaTypeListRequest missing stream index "
                      "(payload_len=%zu) ChannelId=%" PRIu32,
    ...
    guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPECAM received MediaTypeListRequest for stream %d",
                    stream_idx);
    ...
    if (rs &&
        rdpecam_build_media_type_list(rs, media_types, media_type_count)) {
      Stream_SealLength(rs);
      const size_t out_len = Stream_Length(rs);
      guac_rdp_rdpecam_log_stream(client, "TX", ch_name, channel_id, rs);
      pthread_mutex_lock(&(rdp_client->message_lock));
      ...
      guac_client_log(client, GUAC_LOG_DEBUG,
                      "RDPECAM TX ChannelId=%" PRIu32
                      " MessageId=0x0C MediaTypeListResponse (count=%zu)",
  ...
  case RDPECAM_MSG_CURRENT_MEDIA_TYPE_REQUEST: {
    if (!device) {
      guac_client_log(client, GUAC_LOG_WARNING,
                      "RDPECAM CurrentMediaTypeRequest received but no device "
                      "available on ChannelId=%" PRIu32,
                      channel_id);
      return CHANNEL_RC_OK;
    }
    ...
    if (payload_len < 1) {
      guac_client_log(client, GUAC_LOG_WARNING,
                      "RDPECAM CurrentMediaTypeRequest missing stream index "
                      "(ChannelId=%" PRIu32 ")",
                      channel_id);
      return CHANNEL_RC_OK;
    }
    ...
    if (stream_idx == 0) {
        ...
        if (caps && caps->format_count > 0) {
          ...
          media_type = (rdpecam_media_type_desc){
              ...
              .FrameRateDenominator =
                  preferred->fps_den ? preferred->fps_den : 1,
              .PixelAspectRatioNumerator = 1,
              .PixelAspectRatioDenominator = 1,
              .Flags = CAM_MEDIA_TYPE_DESCRIPTION_FLAG_DecodingRequired};
        } else {
      ...
      if (rs && rdpecam_build_current_media_type(rs, &media_type)) {
        ...
        guac_client_log(
            client, GUAC_LOG_DEBUG,
            "RDPECAM TX ChannelId=%" PRIu32
            " MessageId=0x0E CurrentMediaTypeResponse (format=%u, %ux%u@%u/%u)",
  ...
  case RDPECAM_MSG_START_STREAMS_REQUEST: {
    if (!device || !device->sink) {
      guac_client_log(client, GUAC_LOG_WARNING,
                      "RDPECAM StartStreamsRequest received but no device/sink "
                      "available (ChannelId=%" PRIu32 ")",
                      channel_id);
      return CHANNEL_RC_OK;
    }
    ...
    if (payload_len < 1 + 26) { /* stream_idx + media_type_desc */
      guac_client_log(client, GUAC_LOG_WARNING,
                      "RDPECAM StartStreamsRequest too short");
      return CHANNEL_RC_OK;
    }
     ...
     * one, so we must handle the switch automatically. */
    if (rdp_client->rdpecam_sink != NULL &&
      ...
      if (plugin && plugin->devices) {
        /* Iterate through channel slots instead of using broken
         * HashTable_GetKeys */
        for (unsigned int check_idx = 0; check_idx < 100; check_idx++) {
          char check_channel[64];
          snprintf(check_channel, sizeof(check_channel), "RDCamera_Device_%u",
                   check_idx);

          guac_rdpecam_device *old_device =
              (guac_rdpecam_device *)HashTable_GetItemValue(
                  plugin->devices, (void *)check_channel);
       ...
       * 1. It has no device ID, so browser can't distinguish which camera to
       * stop
       * 2. The browser's camera-start handler should gracefully handle
       * switching
       * 3. Sending stop+start creates a temporary "no camera" state that can
       * confuse browser */
      rdp_client->rdpecam_sink = NULL;

      guac_client_log(client, GUAC_LOG_DEBUG,
                      "RDPECAM stopped streaming on old device, proceeding "
                      "with %s (browser will be notified via camera-start)",
                      device->device_name);
    }
    ...
    if (stream_idx == 0) {
      ...
      if (flushed > 0) {
        guac_client_log(client, GUAC_LOG_DEBUG,
                        "RDPECAM flushed %d stale frames before streaming",
                        flushed);
      }
      ...
      guac_client_log(client, GUAC_LOG_DEBUG,
                      "RDPECAM streaming started ChannelId=%" PRIu32
                      " format=%u %ux%u@%u/%u",
                      channel_id, media_type.Format, media_type.Width,
                      media_type.Height, media_type.FrameRateNumerator,
                      media_type.FrameRateDenominator);

      /* Browser pushes frames into rdp_client->rdpecam_sink. Point it at this
       * device's sink. */
      ...
      if (rs && rdpecam_build_start_streams_response(rs, 0)) {
        ...
        guac_client_log(client, GUAC_LOG_DEBUG,
                        "RDPECAM TX ChannelId=%" PRIu32
                        " MessageId=0x01 SuccessResponse (StartStreams)",
                        channel_id);
        ...
        if (result == CHANNEL_RC_OK) {
          ...
          guac_client_for_owner(
              client, guac_rdp_rdpecam_send_camera_start_signal_callback,
              &camera_params);
  ...
  case RDPECAM_MSG_STOP_STREAMS_REQUEST: {
    if (!device || !device->sink) {
      guac_client_log(client, GUAC_LOG_WARNING,
                      "RDPECAM StopStreamsRequest received but no device/sink "
                      "available (ChannelId=%" PRIu32 ")",
                      channel_id);
      return CHANNEL_RC_OK;
    }
    ...
    if (rs && rdpecam_build_stop_streams_response(rs, /*status*/ 0)) {
      ...
      guac_client_log(client, GUAC_LOG_DEBUG,
                      "RDPECAM TX ChannelId=%" PRIu32
                      " MessageId=0x01 SuccessResponse (StopStreams)",
                      channel_id);
  ...
  case RDPECAM_MSG_PROPERTY_LIST_REQUEST: {
    if (!device) {
      guac_client_log(client, GUAC_LOG_WARNING,
                      "RDPECAM PropertyListRequest received but no device "
                      "available (ChannelId=%" PRIu32 ")",
                      channel_id);
      return CHANNEL_RC_OK;
    }
    ...
    guac_client_log(client, GUAC_LOG_DEBUG,
                    "RDPECAM received PropertyListRequest ChannelId=%" PRIu32,
                    channel_id);
    ...
    if (rs) {
      ...
      guac_client_log(client, GUAC_LOG_DEBUG,
                      "RDPECAM TX ChannelId=%" PRIu32
                      " MessageId=0x15 PropertyListResponse (empty)",
                      channel_id);
    }
    break;
  }
  ...
  case RDPECAM_MSG_SAMPLE_REQUEST: {
    if (!device) {
      guac_client_log(client, GUAC_LOG_WARNING,
                      "RDPECAM SampleRequest received but no device available "
                      "on %s[id=%" PRIu32 "]",
                      ch_name, channel_id);
      return CHANNEL_RC_OK;
    }
    ...
    if (payload_len < 1) {
      guac_client_log(client, GUAC_LOG_WARNING,
                      "RDPECAM SampleRequest missing stream index");
      return CHANNEL_RC_OK;
    }
    uint8_t stream_idx;
    Stream_Read_UINT8(stream, stream_idx);
    ...
    if (stream_idx == 0 || stream_idx == device->stream_index) {
      pthread_mutex_lock(&device->lock);
      /* SampleRequests grant credits on the channel they arrive; bind responses
       * there. */
      ...
      if (device->streaming) {
        ...
        if (rdp_client_ctx && !rdp_client_ctx->rdpecam_sink) {
          ...
          guac_client_log(client, GUAC_LOG_DEBUG,
                          "RDPECAM bound session sink to active device due to "
                          "SampleRequest (channel=%" PRIu32 ")",
                          channel_id);
        }
      }
      int queue_size = guac_rdpecam_get_queue_size(device->sink);
      guac_client_log(client, GUAC_LOG_DEBUG,
                      "RDPECAM SampleRequest ChannelId=%" PRIu32
                      " device=%s credits %u->%u queue=%d/%d",
                      channel_id, device->device_name, before, remaining,
                      queue_size, GUAC_RDPECAM_MAX_FRAMES);
    }
    break;
  }
...
static UINT guac_rdp_rdpecam_data(IWTSVirtualChannelCallback *channel_callback,
  ...
  /* Invoke generalized (API-independent) data handler with full callback
   * context */
  guac_rdp_rdpecam_handle_data(rdpecam_channel_callback->client, channel,
                               stream, rdpecam_channel_callback);
 ...
 */
static UINT
guac_rdp_rdpecam_open(IWTSVirtualChannelCallback *channel_callback) {
  ...
  if (strcasecmp(ch_name, GUAC_RDPECAM_CHANNEL_NAME) == 0) {
    ...
    if (s && rdpecam_build_version_request(s)) {
      ...
      pthread_mutex_lock(&(rdp_client->message_lock));
      UINT result =
          channel->Write(channel, (UINT32)out_len, Stream_Buffer(s), NULL);
      pthread_mutex_unlock(&(rdp_client->message_lock));

      Stream_Free(s, TRUE);
      if (result != CHANNEL_RC_OK) {
        guac_client_log(client, GUAC_LOG_ERROR,
                        "Failed to send SelectVersionRequest: %u", result);
        return result;
      }

      /* Remember enumerator channel for DeviceRemovedNotification */
      guac_rdp_rdpecam_channel_callback *cb =
          (guac_rdp_rdpecam_channel_callback *)channel_callback;
      if (cb && cb->plugin) {
      ...
      if (s)
        Stream_Free(s, TRUE);
      guac_client_log(client, GUAC_LOG_ERROR,
                      "Failed to build SelectVersionRequest");
 ...
 */
static UINT
guac_rdp_rdpecam_close(IWTSVirtualChannelCallback *channel_callback) {
  ...
  if (device) {
     ...
     * Only the active sender should trigger browser camera-stop when its
     * channel closes. */
    ...
    if (closing_stream_channel) {
      ...
      /* Only notify browser to stop camera if this device was the active
       * sender. When switching cameras, the old device's stream channel closes
       * but it's no longer the active sender, so we shouldn't send camera-stop.
       */
      if (was_active_sender) {
        guac_client_log(rdpecam_channel_callback->client, GUAC_LOG_DEBUG,
                        "RDPECAM sending camera-stop to browser (stream "
                        "channel closed for active sender device %s)",
        ...
        guac_client_log(rdpecam_channel_callback->client, GUAC_LOG_DEBUG,
                        "RDPECAM NOT sending camera-stop to browser (stream "
                        "channel closed for non-active device %s)",
                        ch_name ? ch_name : "unknown");
      }
    ...
    if (plugin && plugin->devices && ch_name && remaining_refs == 0) {
      ...
      guac_client_log(
          rdpecam_channel_callback->client, GUAC_LOG_DEBUG,
...
static UINT guac_rdp_rdpecam_new_connection(
  ...
  const char *ch_name = rdpecam_listener_callback->channel_name
                            ? rdpecam_listener_callback->channel_name
                            : "rdpecam";
  UINT32 channel_id = 0;
  ...
  guac_client_log(client, GUAC_LOG_DEBUG,
                  "New RDPECAM channel connection (%s) [id=%" PRIu32 "]",
                  ch_name, channel_id);
  ...
  if (strcasecmp(ch_name, GUAC_RDPECAM_CHANNEL_NAME) != 0) {
    ...
    if (!device) {
...
#endif
        guac_client_log(client, GUAC_LOG_ERROR,
                        "Failed to insert RDPECAM device into registry: %s",
                        ch_name);
        guac_rdpecam_device_destroy(device);
        *accept = 0;
        return CHANNEL_RC_OK;
      ...
      guac_client_log(client, GUAC_LOG_DEBUG, "Created new RDPECAM device: %s",
                      ch_name);
    } else {
      pthread_mutex_lock(&device->lock);
      device->ref_count++;
  ...
  *channel_callback = (IWTSVirtualChannelCallback *)rdpecam_channel_callback;
  ...
  guac_client_log(client, GUAC_LOG_DEBUG,
                  "RDPECAM channel connection established (%s) [id=%" PRIu32
                  "]",
                  ch_name, channel_id);
...
static UINT guac_rdp_rdpecam_initialize(IWTSPlugin *plugin,
  ...
  control_listener->parent.OnNewChannelConnection =
      guac_rdp_rdpecam_new_connection;
...
static UINT guac_rdp_rdpecam_terminated(IWTSPlugin *plugin) {
  ...
  if (rdpecam_plugin->devices != NULL) {
    ...
    for (unsigned int check_idx = 0; check_idx < 100; check_idx++) {
      ...
      guac_rdpecam_device *dev = (guac_rdpecam_device *)HashTable_GetItemValue(
          rdpecam_plugin->devices, (void *)channel_name);
      if (dev)
...
guac_rdpecam_device_create(guac_rdp_rdpecam_plugin *plugin,
   ...
   * Note: rdp_client->rdpecam_sink is used as a pointer to the active device's
   * sink for the browser to push frames to. It should NOT be reused by new
   * devices, as doing so would steal the sink from an already-active device. */
  ...
  guac_client_log(plugin->client, GUAC_LOG_DEBUG,
                  "RDPECAM sink created for device: %s", device_name);
  ...
  if (pthread_create(&device->dequeue_thread, NULL,
                     guac_rdp_rdpecam_dequeue_thread, device) != 0) {
    guac_client_log(plugin->client, GUAC_LOG_ERROR,
                    "Failed to create dequeue thread for device: %s",
                    device_name);
  ...
  guac_client_log(plugin->client, GUAC_LOG_DEBUG,
                  "RDPECAM device created: %s (dequeue thread started)",
                  device_name);
...
/**
 * Safely frees a constant device ID key string.
 */
static void guac_rdp_rdpecam_free_id_key(const char *device_id_key) {
  char *ptr = (char *)device_id_key;
  guac_mem_free(ptr);
}
...
guac_rdp_rdpecam_mapping_remove_by_device_id(guac_rdp_rdpecam_plugin *plugin,
  ...
  while (current) {
    if (current->device_id_key &&
        strcmp(current->device_id_key, device_id) == 0) {
      if (plugin->device_id_map)
      ...
      guac_rdp_rdpecam_free_id_key(current->device_id_key);
...
static BOOL guac_rdp_rdpecam_mapping_add(guac_rdp_rdpecam_plugin *plugin,
  ...
  char *id_copy = guac_mem_alloc(strlen(device_id) + 1);
  if (!id_copy) {
    guac_mem_free(channel_copy);
    return FALSE;
  }
  strcpy(id_copy, device_id);

  guac_rdp_rdpecam_device_mapping *entry =
      guac_mem_zalloc(sizeof(guac_rdp_rdpecam_device_mapping));
  if (!entry) {
    guac_mem_free(id_copy);
    guac_mem_free(channel_copy);
    return FALSE;
  }

  entry->device_id_key = id_copy;
  entry->channel_name = channel_copy;
  entry->next = plugin->device_id_mappings;

#ifdef HAVE_WINPR_HASHTABLE_INSERT
  if (!HashTable_Insert(plugin->device_id_map, (void *)id_copy,
                        (void *)entry)) {
#else
  if (HashTable_Add(plugin->device_id_map, (void *)id_copy, (void *)entry) <
      0) {
#endif
    guac_mem_free(id_copy);
    guac_mem_free(channel_copy);
...
UINT DVCPluginEntry(IDRDYNVC_ENTRY_POINTS *pEntryPoints) {
  ...
  guac_rdp_rdpecam_plugin *rdpecam_plugin =
      (guac_rdp_rdpecam_plugin *)pEntryPoints->GetPlugin(
          pEntryPoints, GUAC_RDPECAM_PLUGIN_NAME);
