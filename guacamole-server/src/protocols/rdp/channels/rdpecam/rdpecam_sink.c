...
bool guac_rdpecam_pop(guac_rdpecam_sink* sink, uint8_t** out_buf, size_t* out_len,
    ...
    /* If we need a keyframe, skip all non-keyframes until one is found */
    if (sink->need_keyframe) {
        while (sink->queue_head != NULL && !sink->queue_head->keyframe) {
            guac_rdpecam_frame* dropped = sink->queue_head;
            sink->queue_head = dropped->next;
            if (!sink->queue_head)
                sink->queue_tail = NULL;
            
            sink->queue_size--;
            guac_mem_free(dropped->data);
            guac_mem_free(dropped);
        }

        /* If we drained the queue and still don't have a keyframe, wait again */
        if (sink->queue_size == 0) {
            pthread_mutex_unlock(&sink->lock);
            return guac_rdpecam_pop(sink, out_buf, out_len, out_keyframe, out_pts_ms);
        }

        /* Found a keyframe! */
        sink->need_keyframe = false;
        guac_client_log(sink->client, GUAC_LOG_DEBUG, "RDPECAM found synchronization keyframe");
    }
...
void guac_rdpecam_request_keyframe(guac_rdpecam_sink* sink) {
    ...
    /* Drop any non-keyframes currently at the head of the queue */
    while (sink->queue_head != NULL && !sink->queue_head->keyframe) {
        guac_rdpecam_frame* dropped = sink->queue_head;
        sink->queue_head = dropped->next;
        if (!sink->queue_head)
    ...
    guac_client_log(sink->client, GUAC_LOG_DEBUG, "RDPECAM keyframe requested, purged non-keyframes");

}

void guac_rdpecam_clear(guac_rdpecam_sink* sink) {
    ...
    guac_client_log(sink->client, GUAC_LOG_DEBUG, "RDPECAM sink queue cleared");
