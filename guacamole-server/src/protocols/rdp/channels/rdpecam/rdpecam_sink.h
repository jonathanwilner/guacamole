...
typedef struct guac_rdpecam_sink {
    ...
    /**
     * Whether the sink is waiting for a keyframe before it will allow popping
     * any frames.
     */
    bool need_keyframe;
...
/**
 * Signals that the sink should wait for a keyframe before returning any
 * further frames from guac_rdpecam_pop(). Any non-keyframe frames currently
 * in the queue will be dropped.
 *
 * @param sink
 *     The sink to update.
 */
...
/**
 * Removes all frames currently in the queue, freeing their associated
 * resources.
 *
 * @param sink
 *     The sink to clear.
