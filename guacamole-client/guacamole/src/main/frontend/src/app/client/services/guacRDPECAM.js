...
angular.module('client').factory('guacRDPECAM', ['$injector', function guacRDPECAM($injector) {
    ...
    function startCameraWithParams(client, params, onState) {
        ...
        // If this is the virtual placeholder camera, use the first enabled
        // or available real camera instead.
        if (deviceId === 'default-camera') {
            deviceId = undefined;
            for (var id in cameraRegistry) {
                if (cameraRegistry[id].enabled) {
                    deviceId = id;
                    break;
                }
            }
    ...
    function updateCameraState(clientId, active, deviceId) {
        ...
        // Dispatch DOM event for automation (e.g. Playwright E2E tests)
        window.dispatchEvent(new CustomEvent('guac-camera-state-change', {
            detail: {
                clientId: clientId,
                active: active,
                deviceId: deviceId
